...
bool injection::manualmap::inject(const std::vector<uint8_t>& buffer)
{
	...
	auto exported_hook = this->process.get_module_export(ctx.remote_image, "HookMain");

	// CLEAR HEADERS :)
	auto junk_buffer = std::vector<std::uint8_t>(0x1000);
	std::for_each(junk_buffer.begin(), junk_buffer.end(), [](auto& n) { n = rand() % 0x100; });
	this->process.write_raw_memory(junk_buffer.data(), 0x1000, ctx.remote_image);

	// HOOK QueryPerformanceCounter FOR EXECUTION, THEN UNHOOK TO REMOVE TRACES
	this->process.hook_function("Kernel32.dll", "QueryPerformanceCounter", exported_hook);
...
bool injection::manualmap::map_image(map_ctx& ctx)
{
	...
	logger::log_formatted("Mapping", ctx.image_name);
