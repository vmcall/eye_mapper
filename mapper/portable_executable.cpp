...
import_list portable_executable::get_imports(uintptr_t image_base)
{
	...
	{
		auto name = reinterpret_cast<char*>(image_base + import_table->Name);
		auto entry  = reinterpret_cast<IMAGE_THUNK_DATA64*>(image_base + import_table->OriginalFirstThunk);
		uintptr_t index = 0;

		for (; entry->u1.AddressOfData; index += sizeof(uintptr_t), ++entry)
		{
			...
			import_modules[new_name].emplace_back(data);
		}
	}
